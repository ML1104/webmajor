var BattleScene = new Phaser.Class({
    ...
    nextTurn: function() {  
        ...
        if(this.units[this.index] instanceof PlayerCharacter) {
            // need the player to select action and then enemy
    ...
    checkEndBattle: function() {        
        var victory = true;
        // if all enemies are dead have victory
        for(var i = 0; i < this.enemies.length; i++) {
            if(this.enemies[i].living)
                victory = false;
        }
        var gameOver = true;
        // if all heroes are dead have game over
    ...
    // when the player selected the enemy to be attacked
    receivePlayerSelection: function(action, target) {
        if(action == "attack") {            
            this.units[this.index].attack(this.enemies[target]);              
        }
        // next turn in 0.5 seconds
        this.time.addEvent({ delay: 500, callback: this.nextTurn, callbackScope: this });        
    },    
    endBattle: function() {       
...
var Unit = new Phaser.Class({
    ...
    // notify the menu item when the unit is dead
...
var PlayerCharacter = new Phaser.Class({
    ...
    function PlayerCharacter(scene, x, y, texture, frame, type, hp, damage) {
        Unit.call(this, scene, x, y, texture, frame, type, hp, damage);
        // flip the image so don't have to edit it manually
        this.flipX = true;
...
var ActionsMenu = new Phaser.Class({
    ...
    confirm: function() { 
        // select an action and go to the next menu and choose from the enemies to apply the action
...
var UIScene = new Phaser.Class({
    ...
    {    
        ...
        // for now have only one action so dont send and action id
    ...
    onPlayerSelect: function(id) {
        ...
        // then make actions menu active
        this.heroesMenu.select(id);
        this.actionsMenu.select(0);
        this.currentMenu = this.actionsMenu;
    },
    // have action selected and make the enemies menu active
